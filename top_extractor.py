...
def read_nt_file(data_root, dataset, eid):
    path = '{0}/{1}/{2}/{2}_desc.nt'.format(data_root, dataset, eid)
...
for key in read_source_csv_file("ESBM_plus/elist.txt")[1:]:
    # dataset 1
    nt_file = read_nt_file("ESBM_plus", key[1], key[0])
    find_top(nt_file, key[3], "ESBM_plus", key[1], key[0])


for key in read_source_csv_file("ESBM_benchmark/elist.txt")[1:]:
    # dataset 1
    nt_file = read_nt_file("ESBM_benchmark", key[1], key[0])
    find_top(nt_file, key[3], "ESBM_benchmark", key[1], key[0])
